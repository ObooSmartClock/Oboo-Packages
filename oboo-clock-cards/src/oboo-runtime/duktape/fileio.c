...
char curFileDir[PATH_MAX+1];
char curGlobalFileDir[PATH_MAX];
...
int fileio_readfile_string(duk_context *ctx) {
	...
	getFilePath(curfilename, duk_to_string(ctx, 0), curFileDir);
	if( fileio_push_file_string(ctx, curfilename) == 1) {
		return 1;
	} else {
		duk_pop(ctx);
		getFilePath(curfilename, duk_to_string(ctx, 0), curGlobalFileDir);
		return fileio_push_file_string(ctx, curfilename);
	}
}

duk_function_list_entry fileio_funcs[] = {
	{ "readfile", fileio_readfile_string, 1 },
	{ NULL, NULL, 0 }
};
...
void setDefaultDirectory(char *folderPath){
	sprintf(curFileDir,"%s",folderPath);
}
void setGlobalDirectory(char *folderPath){
	sprintf(curGlobalFileDir,"%s",folderPath);
}