...
void TM16XX::setDisplayDigit(byte digit, byte pos, boolean dot, const byte numberFont[])
{
  // sendChar(pos, numberFont[digit & 0xF], dot);
	// now supports characters as well
	sendChar(pos, numberFont[digit], dot);
}
...
#ifdef TM1618_DEBUG
  char buf[64];
  sprintf(buf, "sendCommand: Writing command 0x%02x\n", cmd);
  Serial.print(buf);
#endif
}
...
#ifdef TM1618_DEBUG
  char buf[64];
  sprintf(buf, "sendData: Writing to addr 0x%02x: 0x%02x\n", 0xC0 | address, data);
  Serial.print(buf);
#endif
}
